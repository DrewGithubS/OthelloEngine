...
const uint64_t THREADSPERBLOCK = 1024;
...
void runGPU() {
    ...
    uint64_t * total_d;
    cudaMalloc(&total_d, sizeof(uint64_t *));
    cudaMemset(total_d, 0, sizeof(uint64_t));

    int dimGrid = (1 + THREADSPERBLOCK - 1)/THREADSPERBLOCK;
    doPerftsOnGPU <<< dimGrid, THREADSPERBLOCK >>> ((uint64_t **) &board.pos.team, &board.pos.lastMoveSkipped, &board.pos.turn, 3, total_d, 1);
    cudaMemcpy(&total, total_d, sizeof(uint64_t), cudaMemcpyDeviceToHost);
